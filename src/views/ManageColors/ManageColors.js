...
const fields_data = ['image','_id','Colours']
...
function handleChangeHeadingColor(e) {
  ...
  const url = "https://yourbuca.com/api/admin/add_color_template"
  ...
  setTemplateFile()
    ...
    return (
        ...
        style={{display: 'none'}} name="pic" accept="image/*" />
                    ...
                    <CButton onClick={()=>addTemplate()} style={{marginTop:20,width:150,marginLeft:10}} block variant="outline" color="success" >Add Color</CButton>

                    </CFormGroup>
                  </CCol>

                </CFormGroup>
                {(response!=null)?response.map(function(d, idx){
            <CDataTable
              items={response}
              fields={fields_data}
              bordered
